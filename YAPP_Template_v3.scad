...
pcbLength           = 74.5; // front to back (X axis)
pcbWidth            = 41; // side to side (Y axis)
pcbThickness        = 1.6; 
standoffHeight      = 5; //-- How much the PCB needs to be raised from the base to leave room for solderings and whatnot
standoffDiameter    = 7;
...
board_z = 1.3;
loc_z = board_z/2 + basePlaneThickness+pcbThickness+standoffHeight+10.5;
loc_x = wallThickness + pcbLength/2 + paddingBack + pcbLength/2 - 51.49/2;
loc_y = wallThickness + pcbWidth/2 + paddingLeft;

translate([loc_x, loc_y, 0]) {
  cylinder(h=100, r=1, $fn=64, center=true);
}
...
//-------------------------------------------------------------------
pcbStands = 
[ [3.8, 3.8, yappFrontLeft, yappHole, yappBaseOnly],
  [3.8, 3.8, yappFrontRight, yappHole, yappBaseOnly],
  [3.8, pcbWidth/2, yappHole, yappBaseOnly],
...
//-------------------------------------------------------------------
cutoutsBase = 
[ [pcbLength/2-1, pcbWidth/2-1, 1, 1, 1, yappCircle]
];

cutoutsLid  = 
[ [29, 12.2, 32, 17.5, 1, yappRectangle],
  [pcbLength - 8, pcbWidth/2-25/2, 12, 25, 1, yappRoundedRect]

];

cutoutsFront =  
[ [15.5-2.5, 15.75 -6, 15, 8, .5, yappRoundedRect]
];
